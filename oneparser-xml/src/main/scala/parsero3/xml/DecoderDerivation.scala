...
private[xml] trait DecoderDerivation {
      ...
      case _: (elem *: elems1) =>
        inline erasedValue[Labels] match {
          case _: (label *: labels1) =>
            ...
            typeClass[elem](ns \ format.formatCase(rootLabel.toString)) match {
              case Right(t) => 
                elems(n) = t
  ...
  inline given derived[T](given ev: Mirror.Of[T], format: TagFormat = TagFormat.LowerCammelCase): Decoder[T] = new Decoder[T] {
