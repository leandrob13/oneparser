...
private[xml] trait EncoderDerivation {
  ...
  inline given derived[T]: (ev: Mirror.Of[T], format: TagFormat = TagFormat.LowerCammelCase) => Encoder[T] = new Encoder[T] {
    def toXml(t: T): NodeSeq = {
      inline ev match {
        case m: Mirror.SumOf[T] =>
          val ord = m.ordinal(t)
          NodeSeq.Empty // No instances for coproducts for now.
        case m: Mirror.ProductOf[T] =>
          encoderCase(t, m, format)
      }
    }
  }
}