...
object Decoder extends DecoderDerivation {
  ...
  given optionFromXml[T]: (fx: Decoder[T]) => Decoder[Option[T]] = instance { ns =>
    if (ns.flatMap(_.child).isEmpty) Right(None)
    else fx.fromXml(ns).map(Some(_))
  }

  given listFromXml[T]: (fx: Decoder[T]) => Decoder[List[T]] = instance { ns =>
  ...
  given mutableSeqFromXml[T]: (fx: Decoder[T]) => Decoder[Seq[T]] = instance { ns =>
